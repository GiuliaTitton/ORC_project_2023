...
for (test_id, test) in  enumerate(tests):
    ...
    for i in range(0, N):
        ...
        ddx_fb = kp * (x_ref - x) + kd * (dx_ref - dx)                # Feedback acceleration
        ddx_des[:,i] = ddx_ref[:,i] + ddx_fb[:,i]                     # Desired acceleration
        Minv = np.linalg.inv(M)                                       # M^-1
        lambda_mat = np.linalg.inv(J.dot(Minv).dot(J.T)) # OS Inertia matrix
        J_moore_pinv = J.T.dot(np.linalg.inv(J.dot(J.T))) 
        
        # secondary task here
        J_T_pinv = lambda_mat.dot(J.dot(Minv))       # Pseudo-inverse of J.T 
        U, S, NJt = np.linalg.svd(J_T_pinv) 
        NJ = NJt.T                # Null space of the pseudo-inverse of J.T
        U, S, NJ_mooret = np.linalg.svd(J_moore_pinv) 
        NJ_moore = NJ_mooret.T # Null space of the Moore Penrose pseudo-inverse of J.T
        ...
        # impedance control (equazione (7) assignment)
        if(test['controller']=='IC_O_simpl'):
            #K_e = np.dot(kp_j ,x_ref-x)  #usando proportional gain dell'end effector task
            #B_e_dot = np.dot(kd_j ,dx_ref-dx)  # usando derivative gain dell'end effector task
            tau[:,i] = h + np.dot(J.T, ddx_fb)  # ddx_fb feedback acceleration calcolata prima
            #-J.T.dot(robot.B.dot(dx[:,i]) + r.stiffness.dot(x[:,i])) + h   

        #impedance control + postural task  (equazione (8) assignment)  
        elif(test['controller']=='IC_O_simpl_post'):
            K_e = np.dot(kp_j,x_ref-x)
            B_e_dot = np.dot(kd_j,dx_ref-dx)
            factor = np.identity(3) - np.dot(J.T,J_T_pinv)
            tau[:,i] = h + np.dot(J.T, K_e + B_e_dot) + np.dot(factor, tau_0) #eq. 8
        
        #exact version of impedance control (equazione (6) assignment)
        elif(test['controller']=='IC_O'):
            law1 = np.dot(kp_j, x_ref-x)
            law2 = np.dot(kd_j, dx_ref - dx)
            mu_1 = np.dot(np.dot(J,Minv),h) - dJdq  # NB: non sono sicura che dJdq sia J_dot * q_dot!!!
            mu = np.dot(lambda_mat, mu_1)  # mu è definito subito sotto l'equazione 2 nell'assignment
            tau[:,i] = np.dot(J.T, law1 + law2 + mu)  # equazione 6
        
        # exact version + postural task. tau_0 must include bias force h!!! (base = equazione 8 dell'assignment)
        elif(test['controller']=='IC_O_post'):                     
            K_e = np.dot(kp_j,x_ref-x)
            B_e_dot = np.dot(kd_j,dx_ref-dx)
            factor = np.identity(3) - np.dot(J.T,J_T_pinv)
            tau[:,i] = h + np.dot(J.T, K_e + B_e_dot) + np.dot(factor, tau_0+h) #eq. 8, considerando che tau_0 contiene anche h         
        
        if(test['controller']=='OSC'):      # Operational Space Control
            tau[:,i] = np.dot(np.dot(np.transpose(J), lambda_mat), ddx_des[:,i]) + h

        '''elif(test['controller']=='IC'):     # Impedance Control (uguale al test 2: impedance control + postural task)
            tau[:,i] = 
